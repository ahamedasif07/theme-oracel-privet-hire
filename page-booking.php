...
<section class="relative flex min-h-[62vh] items-end overflow-hidden pt-32 pb-16">
  ...
  <div class="relative mx-auto w-full max-w-7xl px-6 lg:px-10">
    ...
    <h1 class="mt-4 max-w-3xl font-display text-4xl leading-[1.05] text-foreground md:text-6xl">Book your journey.</h1>
    <p class="mt-5 max-w-2xl text-base text-muted-foreground md:text-lg">
      Complete the form below for an instant fixed-price quote. A confirmation will be sent to your inbox within minutes.
    </p>
  </div>
</section>
  ...
  <div class="mx-auto max-w-4xl">
      ...
      <?php foreach ($stepNames as $i => $s): ?>
        <div class="flex items-center gap-2 sm:gap-4">
          <div class="flex items-center gap-3">
            ...
            <span class="step-label hidden text-xs uppercase tracking-widest sm:inline <?php echo $i === 0 ? 'text-gold' : 'text-muted-foreground'; ?>"><?php echo $s; ?></span>
          </div>
    ...
    <div id="booking-success" class="glass-card hidden rounded-3xl p-12 text-center">
      <div class="mx-auto grid h-16 w-16 place-items-center rounded-full bg-gold text-ink">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6 9 17l-5-5"/></svg>
      </div>
      ...
      <form id="booking-form" class="glass-card space-y-6 rounded-3xl p-8 md:p-12" method="post" action="">
        ...
        <div class="booking-step active grid gap-5 md:grid-cols-2">
          ...
          <div>
            ...
            <input type="number" name="passengers" min="1" max="8" value="1" class="<?php echo $inputCls; ?>" required>
          </div>
          <div class="md:col-span-2">
            <label class="mb-2 block text-xs uppercase tracking-[0.2em] text-muted-foreground">Pickup Address</label>
            <input type="text" name="pickup_address" placeholder="e.g. 10 Downing St, London" class="<?php echo $inputCls; ?>" required>
          </div>
          <div class="md:col-span-2">
            <label class="mb-2 block text-xs uppercase tracking-[0.2em] text-muted-foreground">Destination</label>
            <input type="text" name="destination" placeholder="e.g. Heathrow Terminal 5" class="<?php echo $inputCls; ?>" required>
          </div>
          <div>
        ...
        <div class="booking-step grid gap-4 sm:grid-cols-3">
          ...
          foreach ($vehicleOptions as $i => $v): ?>
            <label class="cursor-pointer rounded-2xl border border-white/10 bg-ink/60 p-6 transition-all hover:border-gold has-[:checked]:border-gold has-[:checked]:bg-gold/5">
              <input type="radio" name="vehicle" value="<?php echo $v['name']; ?>" <?php echo $i === 0 ? 'checked' : ''; ?> class="sr-only">
          ...
          <div class="sm:col-span-3">
            ...
            <textarea name="special_requests" rows="3" placeholder="Child seat, additional stop, etc." class="<?php echo $inputCls; ?>"></textarea>
          </div>
        </div>
        ...
        <div class="booking-step grid gap-5 md:grid-cols-2">
          <div>
            ...
            <input type="text" name="full_name" class="<?php echo $inputCls; ?>" required>
          </div>
          <div>
          ...
          <div class="md:col-span-2">
            <label class="flex items-start gap-3 text-sm text-muted-foreground">
              <input type="checkbox" name="accept_terms" required class="accent-gold mt-1 h-4 w-4">
              <span>I accept the terms &amp; conditions and privacy policy.</span>
            </label>
          </div>
          <?php wp_nonce_field( 'oracle_booking_form', 'oracle_booking_nonce' ); ?>
